-- Person B execution storage. Requires coordinator migration 005, not 006.
-- Parent owns reservation and calls submit only with committed resource IDs.
create table public.subagent_missions (
  mission_id uuid primary key,
  run_id uuid not null,
  event_id uuid not null,
  input jsonb not null,
  status text not null default 'queued' check (status in ('queued','running','waiting','blocked','completed','failed')),
...
create table public.subagent_activity (
...
create index subagent_queue on public.subagent_missions(next_attempt_at) where status in ('queued','waiting','running');
create index subagent_activity_mission on public.subagent_activity(mission_id,id);
alter table public.subagent_missions enable row level security;
alter table public.subagent_activity enable row level security;
revoke all on public.subagent_missions,public.subagent_activity from anon,authenticated;
grant select,insert,update on public.subagent_missions to service_role;
grant select,insert on public.subagent_activity to service_role;
grant usage on sequence public.subagent_activity_id_seq to service_role;
...
begin
  if p_kind='submit' then
    ...
    if (select count(distinct value) from jsonb_array_elements(p_data->'assignedResourceIds')) <> jsonb_array_length(p_data->'assignedResourceIds') or
      exists(select 1 from jsonb_array_elements_text(p_data->'assignedResourceIds') r where not exists(
        select 1 from jsonb_array_elements(snapshot#>'{ambulances,units}') u where u->>'id'=r and u->>'eventId'=p_data->>'eventId' and u->>'status'='assigned')) then
  ...
  if p_kind='claim' then
    select * into job from public.subagent_missions
      where (status in ('queued','waiting') and next_attempt_at<=now()) or (status='running' and lease_until<=now())
