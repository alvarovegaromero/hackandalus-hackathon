...
The next resource/FE boundary is [finite resource state](docs/resource-state-contract.md):
`GET /api/state`, available/total counters and polling every three seconds.
Contract and fixture are ready; endpoint and allocation implementation are pending.
