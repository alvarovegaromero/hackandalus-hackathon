...
- [x] Isolated emergency-drill workspace at `/dashboard/drills`: configurable earthquake/wildfire rehearsals, animated WebGL city, continuous simulation with delayed evacuation arrivals, state reconstruction for 3D replay, no-intervention comparison, local notebook, JSON export and evidence-based lessons reused by matching exercises. See [model and storage limits](docs/emergency-drills.md); no operational state clones or live actions.
- [x] Local drill learning model 3: seeded ad hoc conditions, occupied-team/travel rules, audited observations and replay, Markdown/JSONL, reviewed offline coordinator context and disjoint-family policy evaluation. The candidate regresses on reserved earthquake cases and must not be promoted. See [reproduction, schemas and limits](docs/drills-agent-learning.md). No LLM training, production policy updates or live coordinator integration.
- [x] Drills share the console dark theme and Live operations / Drills tabs; the landing's Learning section presents the rehearse, review, approve and reuse loop. Approved lessons still do not reach the live coordinator.
- [x] Focus the workspace on wildfires: paused preparation, relevant response controls, explicit blocked-team guidance, source-linked approved lessons before and during matching rehearsals, multi-metric comparison and wildfire-scoped frozen evaluation. Historical engine semantics remain intact; this reuses reviewed experience in the local training workspace, not in the production coordinator.
