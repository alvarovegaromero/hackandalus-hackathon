...
`lib/event-pipeline.ts` → `GET /api/telemetry` → `/telemetry`.
...
With Node 22.21+ (22.x), run `npm ci`, then `npm run dev`. Open
<http://localhost:3000/telemetry>. In another terminal run `npm run mock:events`.
...
`EVENT_API_URL` overrides the script's base URL. The script loads `.env.local`
and uses `CRISIS_API_TOKEN` if configured. The viewer has a password input for
that token; it is kept in component memory, never in localStorage or a URL.
Both routes require the bearer token when set. Without a token they are open
in development; in production they return 503. No external communications or
model calls are triggered by this pipeline.
...
```json
{
  ...
  "description": "Señal simulada enviada por HTTP",
  "zoneId": "zone-south",
  "category": "incendio",
  "severity": "high",
  "confidence": "high",
...
The strict schema is `incomingEventSchema` in `lib/validation.ts`. The UUID `id`
...
- Telemetry currently contains input payloads. Operator authentication and
  incident-specific access control must replace the shared demo token before
  using private incident data.

Verification: automated tests cover ingestion, validation, ID deduplication,
conflicts, live delivery, replay, cursor precedence, expiration, auth and stream
