...
import { valueAt, withKeyframe, type FactDef, type FactValue, type Keyframe, type LabeledSignal, type TruthLabel } from "./world";
...
function report(pack: ScenarioPack, r: () => number, source: Source, claim: Claim, value: FactValue, id: string, atMin: number): Signal | null {
  ...
  const body: Signal["body"] = source.channel === "sensor"
    ? { type: "reading", metric: claim.kind, value, ...(claim.unit ? { unit: claim.unit } : {}) }
    : { type: "text", text: render(pick(r, pack.templates[claim.kind]), { entity: claim.entityLabel, value: String(value), place: location.placeName }) };
  ...
  for (const effect of event.effects as Effect[]) {
    if (effect.type === "set_fact") {
      ...
      for (let i = 0; i < effect.count; i++) {
        ...
        emit(report(pack, r, source, fact, value, `${event.id}-${n++}`, atMin), { truth: truthLabel, eventId: event.id, factId: fact.id });
        if (!wrong) sawTruth = true;
      }
    } else {
      for (let i = 0; i < effect.count; i++) {
        const source = sourceOf(pack, pick(r, effect.sourceIds));
        emit(report(pack, r, source, effect, effect.value, `${event.id}-${n++}`, atMin), { truth: "hoax", eventId: event.id });
      }
    }
  }
  return { ...state, facts, fired: [...state.fired, event.id], pending: [...state.pending, ...made] };
}
...
function flush(state: EngineState): { state: EngineState; emitted: LabeledSignal[] } {
  ...
  return { state: { ...state, pending: state.pending.filter((l) => l.signal.receivedAtMin > state.nowMin) }, emitted: due };
}
...
export function fire(pack: ScenarioPack, state: EngineState, eventId: string, atMin = state.nowMin): Step {
...
export function probe(pack: ScenarioPack, state: EngineState, factId: string, sourceId: string, atMin: number): LabeledSignal | null {
