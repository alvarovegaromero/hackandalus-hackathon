...
export const scenarioPackSchema = z.object({
  ...
  unique(pack.facts.map((f) => f.id), "fact");
  unique(pack.sources.map((s) => s.id), "source");
  unique(pack.events.map((e) => e.id), "event");
  ...
  for (const event of pack.events) {
    for (const effect of event.effects) {
      if (effect.type !== "hoax" && !facts.has(effect.factId)) fail(`${event.id}: unknown fact ${effect.factId}`);
  ...
  for (const s of pack.sources) if (s.delayMin[0] > s.delayMin[1]) fail(`source ${s.id}: delayMin is reversed`);
});
